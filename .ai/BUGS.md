...
## P1 — 즉시 수정 (사용성 직결)
1. **업로드 중 Activity 재생성 시 UI 초기화** — 서비스는 살아있고 알림창에 17% 표시되지만, 앱 UI는 대기 상태로 돌아감. restoreState()에서 isRunning=true인데도 대기 상태로 렌더링되는 버그. 재현: 업로드 중 다른 앱 사용 후 복귀 또는 시스템 메모리 부족으로 Activity destroy
2. **업로드 속도 느림 (0.5MB/s)** — WiFi에서도 느림. 스킵 시 ZIP 전체 스트리밍, 이어하기 시 매 파일 OneDrive HTTP 호출. 스킵 최적화 필요
3. **대용량 동영상 업로드 중 앱 멈춤/끊김** — 청크 업로드 실패 시 재시도 없이 즉시 실패. 네트워크 불안정 시 Worker 블록되면 전체 파이프라인 정지

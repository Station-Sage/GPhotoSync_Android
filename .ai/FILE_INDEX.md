...
| 파일 | 줄수 | 역할 | 수정빈도 |
|------|------|------|----------|
| TakeoutTabHelper.kt | 562 | Takeout 탭 UI, 상태관리, 공통함수 | 높음 |
| OneDriveApi.kt | 544 | MS Graph API (폴더/파일/청크/번들) | 중간 |
| SyncForegroundService.kt | 398 | 실시간 동기화 서비스 | 낮음 |
| MainActivity.kt | 376 | 탭 관리, API 설정, 인증 백업 | 중간 |
| TakeoutUploadPipeline.kt | 357 | 업로드 파이프라인 (Producer/Worker) | 높음 |
| TakeoutUploadService.kt | 331 | Takeout 서비스 본체 (상태저장, 알림, 로그) | 중간 |
| TakeoutOperations.kt | 268 | analyzeZip, organizeAlbums, migrateMonthToYear | 중간 |
| SyncTabHelper.kt | 241 | 동기화 탭 UI | 낮음 |
| OAuthActivity.kt | 207 | WebView OAuth (Google/MS) | 낮음 |
...
| TakeoutUtils.kt | 117 | 데이터 클래스, 미디어 판별, JSON 파싱, 날짜 유틸 | 낮음 |

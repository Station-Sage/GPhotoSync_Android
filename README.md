...
### Claude Code 사용 시
프로젝트 루트의 `CLAUDE.md`가 자동으로 로드됩니다. `.ai/WORKFLOW.md` → `.ai/CURRENT_TASK.md` 순서로 읽고 작업을 시작합니다.

### Genspark 등 기타 AI 사용 시
이 README.md를 읽은 후 `.ai/` 폴더의 md 파일들을 읽어주세요.
...
- **OneDriveApi.kt** — Microsoft Graph API 클라이언트 (suspend 함수)
- **OneDriveUploader.kt** — OneDrive 업로드 (simple/chunked)
...
Takeout ZIP의 앨범 정보로 OneDrive에 앨범을 생성합니다. OneDrive Bundle API(POST /drive/bundles)를 사용하며 용량은 0입니다. 20개씩 분할 생성하고, 동일 이름 앨범이 있으면 POST /drive/bundles/{id}/children으로 아이템을 추가합니다. 중복 아이템은 자동 스킵됩니다.
...
#### OneDriveApi.kt (리팩토링)
- 전체 callback → suspend 함수 전환 (suspendCancellableCoroutine)
- OneDriveFiles.kt 삭제, 기능을 OneDriveApi.kt로 통합
- OneDriveUploader.kt 신규 생성 (업로드 로직 분리)
- Thread.sleep → coroutine delay 교체
- TakeoutUploadService.kt의 11개 suspend 래퍼 직접 호출로 교체

#### B-002: moveFile 409 수정
- moveFile에서 409 (대상에 이미 존재) 시 성공으로 처리 + 스킵 로그

#### B-003: 앨범 정리 수정
- createAlbum batchSize 150 → 20 (OneDrive 배치 제한)
- addChild URL: /me/drive/items/{bid}/children → /me/drive/bundles/{bid}/children
- addChild 400 "already exists" 스킵 처리
- addChild 네트워크 에러(resp=0) 시 재시도 3회 + 딜레이(2s/4s/6s)

...
- **MainActivity.kt** — 탭 기반 메인 UI (동기화, 인증, 정보, Takeout)
- **SyncTabHelper.kt** — Sync 탭 UI 로직
- **TakeoutTabHelper.kt** — Takeout 탭 UI 로직 (ZIP 선택, 분석, 업로드, 앨범정리, 마이그레이션)
...
- **ensureFolder** — 폴더 경로 재귀 생성 (POST /me/drive/{parent}/children, conflictBehavior: fail)
...
**탭 1 — 인증**: Google/Microsoft OAuth 설정 및 로그인. Client ID 직접 입력 또는 JSON 파일 가져오기. 인증 정보 내보내기/가져오기 및 전체 초기화.
...
- **app_settings** — 테마 설정 (0: 시스템, 1: 라이트, 2: 다크)

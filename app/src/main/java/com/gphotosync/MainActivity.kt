...
class MainActivity : AppCompatActivity() {
    ...
    private fun showGoogleSetupDialog() {
        ...
        val rbJson = RadioButton(this).apply { text = "JSON 파일에서 가져오기" }
    ...
    private fun showMsSetupDialog() {
        ...
        AlertDialog.Builder(this).setTitle("Microsoft API 설정").setView(layout)
            .setPositiveButton("저장") { _, _ ->
                ...
                if (id.isNotEmpty()) {
                    TokenManager.save(TokenManager.KEY_MS_CLIENT_ID, id); updateAuthUI()
                    Toast.makeText(this, "Microsoft API 설정 완료", Toast.LENGTH_SHORT).show()
                } else { Toast.makeText(this, "Client ID를 입력하세요", Toast.LENGTH_SHORT).show() }
            }.setNegativeButton("취소", null).show()
    }
